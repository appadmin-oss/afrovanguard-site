...
/**
 * Real client IP, proxy-aware. Forwarded headers (CF-Connecting-IP /
 * X-Forwarded-For) are trusted ONLY when the direct peer is a Cloudflare edge
 * (or an operator-listed proxy via STS_TRUSTED_PROXIES). Otherwise they are
 * attacker-controlled and ignored — without this, the per-IP rate limits below
 * are reset to zero just by rotating an X-Forwarded-For header.
 */
function sts_client_ip() {
    ...
    $trusted = $cf;
    foreach (explode(',', (string)getenv('STS_TRUSTED_PROXIES')) as $c) {
        $c = trim($c); if ($c !== '') $trusted[] = $c;
    }
    $peerTrusted = false;
